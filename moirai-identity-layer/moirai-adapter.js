/*
  Moirai Specialist Adapter
  Version: 0.1.0
  Architecture: Noema -> NAIB -> Moirai -> The Refrain
...
export const MoiraiAdapter = Object.freeze({
  id: "moirai",
  name: "Moirai",
  version: "0.1.0",
  ...
  canExecute() {
    return {
      ...
      note:
        "Moirai can prepare musical guidance and Refrain handoffs, but cannot claim that an external performance, recording, publishing action or studio process was executed."
    };
  },
  ...
  prepare(context = {}) {
    ...
    return {
      ...
      authorship: {
        owner: "user",
        principle:
          "Moirai helps the musician develop the work without claiming authorship, copyright ownership or creative identity."
      },
      executionClaims: {
...
export function getMoiraiDomains(message = "") {
  return detectDomains(message);
}

export function getMoiraiResourceHints(message = "", limit = 4) {
  const domains = detectDomains(message);
...
export function getMoiraiModes() {
  return [...MODES];
}

export default MoiraiAdapter;
